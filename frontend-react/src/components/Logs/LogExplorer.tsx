...
function sevBadgeClass(sev: string) {
  ...
  if (sev === 'debug') return 'bg-[#475569]/30 text-[#94A3B8]';
...
function sevTextClass(sev: string) {
  ...
  if (sev === 'debug') return 'text-[#64748B]';
  return 'text-[#CBD5E1]';
}
    ...
    <button
      ...
      className={`text-left rounded-xl border p-4 transition-all duration-200 hover:shadow-xl backdrop-blur-sm
        ${active
          ...
          : 'bg-white/[0.06] border-white/[0.08] hover:bg-white/[0.10]'
        }`}
    >
      <p className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-widest mb-1">{label}</p>
      ...
      <div className="relative rounded-2xl overflow-hidden bg-[#0F172A]">
        ...
        <div className="relative px-6 pt-6 pb-16">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-1.5">
                ...
                <div>
                  <h1 className="font-display text-2xl font-bold text-white tracking-tight leading-none">Log Explorer</h1>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-[10px] text-[#64748B]">LogQL · SSH-proxied · {since} window</span>
                  </div>
                </div>
              </div>
              <p className="text-[#64748B] text-xs ml-[48px]">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-[#94A3B8] bg-white/[0.06] hover:bg-white/[0.10] transition-colors border border-white/[0.08] disabled:opacity-40"
              >
                <Download size={13} /> Export
              </button>
              <button
                onClick={() => refetch()}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-[#94A3B8] bg-white/[0.06] hover:bg-white/[0.10] transition-colors border border-white/[0.08]"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
            ...
            <SevCard label="Debug"    count={counts.debug} colorClass="text-[#94A3B8]" active={severityFilter === 'debug'} onClick={() => setSeverityFilter(severityFilter === 'debug' ? null : 'debug')} />
          </div>
        </div>
      </div>
        ...
        <div className="overflow-auto max-h-[680px]">
          {isLoading ? (
            ...
            <div className="py-20 text-center px-8">
              ...
              <p className="text-[#94A3B8] font-semibold mb-1">
              ...
              <p className="text-[#334155] text-xs mt-2 font-mono">
            ...
            <div className="py-20 text-center px-8">
              ...
              <p className="text-[#94A3B8] font-semibold mb-1">Loki Unavailable</p>
              <p className="text-[#475569] text-sm max-w-sm mx-auto">
              ...
              <p className="text-[#334155] text-xs mt-2 font-mono">{data.error}</p>
            </div>
          ) : filteredLogs.length === 0 ? (
            <div className="py-24 text-center px-8">
              <FileText size={32} className="text-[#1E293B] mx-auto mb-3" />
              <p className="text-[#475569] text-sm font-semibold">No log entries matched</p>
              <p className="text-[#334155] text-xs mt-1 mb-4">Try widening the time range, using <code className="text-[#34D399] bg-[#059669]/10 px-1 rounded">{'{}' }</code> to show all logs, or open the <strong className="text-[#64748B]">Labels</strong> browser to explore available streams.</p>
            ...
            <table className="w-full text-[12px] font-mono border-collapse">
              <tbody>
                {filteredLogs.map((l: any, i: number) => {
                  ...
                  return (
                    <tr key={i} className={`border-b border-[#0F172A] hover:bg-white/[0.025] group ${sevRowClass(sev)}`}>
                      {/* Line number */}
                      <td className="px-2 py-0.5 text-[#334155] select-none text-right w-10 align-top tabular-nums">
                        {i + 1}
                      </td>
                      {/* Timestamp */}
                      ...
                      <td className={`px-2 py-0.5 break-all align-top ${sevTextClass(sev)}`}>
                        ...
                        {l.labels && Object.keys(l.labels).length > 0 && (
                          <span className="ml-2 text-[#1E293B] opacity-0 group-hover:opacity-100 transition-opacity text-[10px]">
