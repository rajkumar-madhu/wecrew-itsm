      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            ...
            <a href={incident.htmlUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-stone-800 group-hover:text-indigo-700 transition-colors truncate max-w-[380px]">
              {incident.title}
            </a>
          </div>
...
function ServiceCard({ service }: { service: PdService }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-4 shadow-sm hover:shadow-md transition-all">
      ...
      {service.htmlUrl && (
        <a href={service.htmlUrl} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1 text-[10px] text-stone-400 hover:text-indigo-600 transition-colors">
        ...
        <div className="relative px-6 py-5">
              ...
              <div className="flex items-center gap-3 mb-1.5">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <h1 className="font-display text-2xl font-bold text-white tracking-tight">PagerDuty</h1>
                    ...
                    {statusData?.accountName && (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/[0.06] border border-white/[0.08] text-[10px] font-mono text-white/50">
                        <Building2 size={9} /> {statusData.accountName}
                      </span>
                    )}
                  </div>
                  <p className="text-[#94A3B8] text-xs">Incidents · Services · On-Call · Escalation Policies</p>
                </div>
              </div>
            </div>
            <button
              onClick={() => refetchOverview()}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/[0.06] border border-white/[0.08] text-[11px] text-white/50 hover:bg-white/[0.10] transition-all"
            >
          ...
          <div className="mt-4 ml-[52px] flex items-center gap-3 flex-wrap">
            {[
              ...
              <div key={pill.label} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[11px]">
                  ...
                  <div className="space-y-2 ml-5">
                    {group.layers.sort((a, b) => a.level - b.level).map((layer, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <span className="shrink-0 w-6 h-6 rounded-full bg-indigo-50 border border-indigo-200 flex items-center justify-center text-[9px] font-bold text-indigo-600">L{layer.level}</span>
              ...
              {policiesList.map((policy: any) => (
                <div key={policy.id} className="px-5 py-4">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="text-[9px] font-mono px-2 py-0.5 rounded bg-indigo-50 text-indigo-600">
        ...
        <div className="space-y-4 max-w-xl">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-5">
            <h3 className="text-sm font-bold text-stone-800 mb-2 flex items-center gap-2">
              <Link2 size={14} className="text-indigo-500" /> Incoming Webhook URL
            </h3>
