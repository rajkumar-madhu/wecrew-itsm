...
function EditChangeModal({ change, onClose }: { change: any; onClose: () => void }) {
  ...
  const darkInputStyle: React.CSSProperties = { background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#E2EEF9', borderRadius: '8px', padding: '8px 12px', width: '100%', fontSize: '14px' };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="w-full max-w-lg p-6 space-y-4 shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto rounded-2xl" style={{ background: '#0A0514', border: '1px solid rgba(124,58,237,0.3)' }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold" style={{ color: '#E2EEF9' }}>Edit Change</h3>
          <button onClick={onClose} className="p-1 rounded-lg" style={{ color: 'rgba(255,255,255,0.4)' }}><X size={18} /></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Short Description *</label>
            <input value={form.shortDescription} onChange={(e) => setForm({ ...form, shortDescription: e.target.value })} style={darkInputStyle} />
          </div>
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Description</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Type</label>
              <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })} style={darkInputStyle}>
                {CHANGE_TYPES.map((v) => <option key={v} value={v} style={{ background: '#0A0514' }}>{v}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Risk</label>
              <select value={form.risk} onChange={(e) => setForm({ ...form, risk: e.target.value })} style={darkInputStyle}>
                {RISK_LEVELS.map((v) => <option key={v} value={v} style={{ background: '#0A0514' }}>{v}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} style={darkInputStyle}>
                <option value="" style={{ background: '#0A0514' }}>Select...</option>
                {CATEGORIES.map((v) => <option key={v} value={v} style={{ background: '#0A0514' }}>{v}</option>)}
              </select>
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Justification</label>
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button onClick={onClose} className="px-3 py-1.5 rounded-lg text-sm" style={{ color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.1)' }}>Cancel</button>
...
function AssignChangeModal({ change, onClose }: { change: any; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="w-full max-w-md p-6 space-y-4 shadow-2xl animate-fade-in rounded-2xl" style={{ background: '#0A0514', border: '1px solid rgba(124,58,237,0.3)' }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold" style={{ color: '#E2EEF9' }}>Assign Change</h3>
          <button onClick={onClose} className="p-1 rounded-lg" style={{ color: 'rgba(255,255,255,0.4)' }}><X size={18} /></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Assignment Group</label>
            <select value={selectedTeam} onChange={(e) => { setSelectedTeam(e.target.value); setSelectedUser(''); }} style={darkInputStyle}>
              <option value="" style={{ background: '#0A0514' }}>Select team...</option>
              {teams.map((t: any) => <option key={t.id} value={t.id} style={{ background: '#0A0514' }}>{t.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Assign To</label>
            <select value={selectedUser} onChange={(e) => setSelectedUser(e.target.value)} style={darkInputStyle}>
              <option value="" style={{ background: '#0A0514' }}>Select member...</option>
              {members.map((m: any) => {
                const user = m.user || m;
                return <option key={user.id} value={user.id} style={{ background: '#0A0514' }}>{user.firstName} {user.lastName}</option>;
              })}
            </select>
          </div>
        </div>
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-20 mt-1 w-48 rounded-lg py-1 shadow-xl animate-fade-in" style={{ background: '#0A0514', border: '1px solid rgba(124,58,237,0.3)' }}>
            {allowed.map((s) => (
              <button key={s} onClick={() => handleTransition(s)} className="w-full text-left px-3 py-2 text-sm transition-colors" style={{ color: '#CBD5E1' }}
...
export default function ChangeDetail() {
  ...
  return (
    <div className="animate-fade-in space-y-0" style={{ background: 'linear-gradient(180deg, #0A0514 0%, #08041C 40%, #060606 100%)', minHeight: '100vh', margin: '-1.5rem', padding: '1.5rem' }}>
      {/* ── HERO BANNER ── */}
      <div className="relative rounded-2xl overflow-hidden mb-5" style={{ background: 'linear-gradient(135deg, #0A0514 0%, #08041C 100%)' }}>
        ...
        <div className="relative px-6 py-5">
          <div className="flex items-center gap-2 mb-4">
            <button onClick={() => navigate('/changes')} className="flex items-center gap-1.5 text-slate-400 hover:text-white text-sm transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="flex-1">
              ...
              <h1 className="text-xl font-display font-bold text-white">{chg.shortDescription}</h1>
              <div className="flex items-center gap-4 mt-3 text-sm text-slate-400">
                ...
                {scheduledStart && <span className="flex items-center gap-1.5"><Calendar size={14} className="text-indigo-400" /> {formatDate(scheduledStart)}</span>}
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={() => setShowEditModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.06] hover:bg-white/[0.1] text-slate-300 text-xs font-medium rounded-lg transition-all border border-white/[0.08]">
                <Pencil size={13} /> Edit
              </button>
              <button onClick={() => setShowAssignModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.06] hover:bg-white/[0.1] text-slate-300 text-xs font-medium rounded-lg transition-all border border-white/[0.08]">
              ...
              {chg.state === 'APPROVAL' && (
                <>
                  ...
                  <button onClick={handleApprove} disabled={approveLoading} className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-500 to-violet-500 text-white text-xs font-semibold rounded-lg shadow-lg transition-all">
      ...
      {/* Tabs */}
      <div className="flex gap-1 px-2 rounded-t-xl" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)' }}>
        {tabs.map((tab) => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} className="px-4 py-2.5 text-sm font-medium transition-colors relative" style={{ color: activeTab === tab.key ? '#C4B5FD' : 'rgba(255,255,255,0.4)' }}>
            {tab.label}
            {'count' in tab && tab.count > 0 && <span className="ml-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.25)' }}>({tab.count})</span>}
      ...
      {activeTab === 'details' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <h3 className="text-sm font-medium mb-3" style={{ color: 'rgba(255,255,255,0.40)' }}>Description</h3>
            <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: '#CBD5E1' }}>{chg.description}</p>
          </div>
          <div className="space-y-4">
            <div className="rounded-xl p-5 space-y-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <h3 className="text-sm font-medium mb-2" style={{ color: 'rgba(255,255,255,0.40)' }}>Properties</h3>
              {[
                ...
                <div key={label} className="flex justify-between text-sm">
                  ...
                  <span className="font-mono text-xs" style={{ color: '#CBD5E1' }}>{value}</span>
                </div>
              ))}
            </div>
            <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <h3 className="text-sm font-medium mb-3" style={{ color: 'rgba(255,255,255,0.40)' }}>Configuration Items</h3>
              <div className="space-y-1.5">
                {configItems.length > 0 ? configItems.map((ci, idx) => (
                  <div key={ci + idx} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-mono text-xs" style={{ color: '#CBD5E1' }}>{ci}</span>
                  </div>
                )) : (
                  <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>No configuration items</p>
                )}
              </div>
            </div>
          </div>
      ...
      {activeTab === 'plans' && (
        <div className="space-y-6">
          {[
            ...
            <div key={plan.title} className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <h3 className="text-sm font-medium mb-3 flex items-center gap-2" style={{ color: 'rgba(255,255,255,0.40)' }}>
                <plan.icon size={14} /> {plan.title}
              </h3>
              <pre className="text-sm leading-relaxed whitespace-pre-wrap font-mono p-4 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', color: '#CBD5E1', border: '1px solid rgba(255,255,255,0.06)' }}>{plan.content || 'No plan provided'}</pre>
            </div>
          ))}
        </div>
      )}
      ...
      {activeTab === 'approvals' && (
        <div className="space-y-4">
          {approvals.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'rgba(255,255,255,0.4)' }}>No approvals found</p>
            ...
            return (
              <div key={approval.id} className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${borderColor}` }}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: approval.status === 'APPROVED' ? 'rgba(5,150,105,0.15)' : approval.status === 'REJECTED' ? 'rgba(220,38,38,0.15)' : 'rgba(217,119,6,0.15)' }}>
                      {approval.status === 'APPROVED' ? <CheckCircle size={16} style={{ color: '#6EE7B7' }} /> : approval.status === 'REJECTED' ? <XCircle size={16} style={{ color: '#FCA5A5' }} /> : <Clock size={16} style={{ color: '#FCD34D' }} />}
                    </div>
                    <div>
                      <p className="text-sm font-medium" style={{ color: '#CBD5E1' }}>{approverName}</p>
                      <p className="text-xs" style={{ color: 'rgba(255,255,255,0.25)' }}>{approval.decidedAt ? formatDate(approval.decidedAt) : 'Pending'}</p>
                    </div>
                  </div>
                  <span className="badge text-[10px] px-2 py-0.5 rounded-md" style={approval.status === 'APPROVED' ? { background: 'rgba(5,150,105,0.15)', color: '#6EE7B7', border: '1px solid rgba(5,150,105,0.3)' } : approval.status === 'REJECTED' ? { background: 'rgba(220,38,38,0.15)', color: '#FCA5A5', border: '1px solid rgba(220,38,38,0.3)' } : { background: 'rgba(217,119,6,0.15)', color: '#FCD34D', border: '1px solid rgba(217,119,6,0.3)' }}>{approval.status}</span>
                </div>
                {approval.comments && <p className="text-sm mt-2 pl-11" style={{ color: 'rgba(255,255,255,0.4)' }}>{approval.comments}</p>}
              </div>
            );
          })}
        </div>
      ...
      {activeTab === 'timeline' && (
        <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
          {timeline.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'rgba(255,255,255,0.4)' }}>No timeline events</p>
          ) : (
            <div className="relative">
                ...
                {timeline.map((event) => {
                  ...
                  return (
                    <div key={event.id} className="flex gap-4 relative">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center z-10" style={{ background: 'rgba(124,58,237,0.2)', color: '#C4B5FD' }}>
                        <Activity size={14} />
                      </div>
                      <div className="flex-1 pt-1">
                        <p className="text-sm" style={{ color: '#CBD5E1' }}>{eventDescription}</p>
                        <div className="flex items-center gap-3 mt-1 text-xs" style={{ color: 'rgba(255,255,255,0.25)' }}>
