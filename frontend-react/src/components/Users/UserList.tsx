...
const roleBadgeConfig: Record<Role, { cls: string; icon: any; label: string }> = {
  ...
  VIEWER:   { cls: 'bg-[#F1F5F9] text-[#64748B] border-[#CBD5E1]', icon: Users,    label: 'Viewer' },
};
...
const statusConfig: Record<UserStatus, { dot: string; icon: any; label: string; cls: string }> = {
  ...
  INACTIVE: { dot: 'bg-[#94A3B8]', icon: XCircle,     label: 'Inactive', cls: 'text-[#94A3B8]' },
...
function ActionsDropdown({ user, onEdit, onToggleLock }: { user: User; onEdit: (u: User) => void; onToggleLock: (u: User) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(!open)} className="p-1.5 rounded-lg text-[#94A3B8] hover:text-[#0F172A] hover:bg-[#F1F5F9] transition-colors" title="More actions">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-[#E2E8F0] rounded-xl py-1 shadow-xl z-30 animate-fade-in">
          <button onClick={() => { onEdit(user); setOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-[#334155] hover:bg-[#F8FAFC] transition-colors">
            <Pencil className="w-3.5 h-3.5 text-[#94A3B8]" /> Edit User
          </button>
          <button onClick={() => { onToggleLock(user); setOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-[#334155] hover:bg-[#F8FAFC] transition-colors">
          ...
          <button className="flex items-center gap-2 w-full px-3 py-2 text-sm text-[#334155] hover:bg-[#F8FAFC] transition-colors">
  ...
  if (isError) {
    return (
      <div className="animate-fade-in space-y-6">
        <div className="relative rounded-2xl overflow-hidden bg-[#0F172A] p-6">
          <h1 className="font-display text-2xl font-bold text-white flex items-center gap-3"><Shield className="w-6 h-6 text-[#818CF8]" /> User Management</h1>
        </div>
        <div className="flex flex-col items-center justify-center h-64 text-[#EF4444]">
          ...
          <p className="text-sm text-[#94A3B8] mt-1">{(error as any)?.response?.data?.error || 'Check your connection.'}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in space-y-0">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-[#0F172A]">
        ...
        <div className="absolute top-0 right-0 w-80 h-80 bg-[#6366F1]/8 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
        ...
        <div className="relative px-6 py-5">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2.5 mb-1.5">
                ...
                <h1 className="font-display text-2xl font-bold text-white tracking-tight">User Management</h1>
                <span className="text-[9px] font-mono font-bold text-[#818CF8] bg-[#6366F1]/15 px-1.5 py-0.5 rounded border border-[#6366F1]/20">IAM</span>
              </div>
              <p className="text-[#94A3B8] text-sm ml-[42px]">
                {isLoading ? 'Loading users...' : <>Manage accounts, roles & permissions across your organization &middot; <span className="font-mono text-white font-bold">{totalCount}</span> users</>}
              </p>
            </div>
          ...
          <div className="flex items-center gap-3 mt-4 ml-[42px]">
            {ALL_ROLES.map(role => {
              ...
              return (
                <div key={role} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.06]">
                  <RoleIcon className="w-3.5 h-3.5 text-[#94A3B8]" />
                  <div>
                    <p className="text-sm font-bold text-white font-display">{roleCounts[role] || 0}</p>
                    <p className="text-[9px] text-[#64748B] uppercase tracking-wider">{cfg.label}</p>
                  </div>
                </div>
              );
            })}
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              ...
              <div>
                <p className="text-sm font-bold text-white font-display">{users.filter(u => u.mfaEnabled).length}</p>
                <p className="text-[9px] text-[#64748B] uppercase tracking-wider">MFA On</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-[#FAFBFC] border border-[#E2E8F0] rounded-lg text-sm text-[#0F172A] placeholder-[#94A3B8] focus:outline-none focus:border-[#6366F1] focus:ring-2 focus:ring-[#6366F1]/10 transition-all"
            />
          </div>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={clsx('px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all border',
                roleFilter === 'ALL' ? 'bg-[#0F172A] text-white border-[#0F172A]' : 'bg-white text-[#94A3B8] border-[#E2E8F0] hover:border-[#CBD5E1]'
              )}
            >
              All Roles
            </button>
            {ALL_ROLES.map(role => {
              ...
              return (
                <button
                  ...
                  className={clsx('px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all border',
                    roleFilter === role ? `${cfg.cls}` : 'bg-white text-[#94A3B8] border-[#E2E8F0] hover:border-[#CBD5E1]'
                  )}
                >
                  {cfg.label}
                </button>
            ...
            return (
              <button
                ...
                className={clsx('flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all border',
                  statusFilter === s ? `bg-[#F8FAFC] ${sc.cls} border-current` : 'bg-white text-[#94A3B8] border-[#E2E8F0] hover:border-[#CBD5E1]'
                )}
              >
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-[#94A3B8] hover:text-[#EF4444] hover:bg-[#FEF2F2] rounded-lg border border-transparent hover:border-[#FECACA] transition-all">
              <X size={13} /> Clear
            </button>
          )}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#E2E8F0] bg-[#FAFBFC]">
                <th className="px-5 py-3 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">User</th>
                <th className="px-5 py-3 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Role</th>
                <th className="px-5 py-3 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Status</th>
                <th className="px-5 py-3 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Organization</th>
                <th className="px-5 py-3 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Department</th>
                <th className="px-5 py-3 text-left text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Last Login</th>
                <th className="px-5 py-3 text-center text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">MFA</th>
                <th className="px-5 py-3 text-right text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-5 py-16 text-center">
                    <div className="flex flex-col items-center gap-3 text-[#94A3B8]">
                      ...
                      <p className="text-lg font-medium text-[#64748B]">No users found</p>
                ...
                users.map(user => {
                  ...
                  return (
                    <tr key={user.id} className={clsx('border-b border-[#F1F5F9] hover:bg-[#FAFBFC] transition-colors group', isCurrentUser && 'bg-[#EEF2FF]/30')}>
                      ...
                      <td className="px-5 py-3.5">
                        {org ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs text-[#334155] font-medium truncate max-w-[120px]">{org.name}</span>
                            {org.environment && (
                          ...
                          <span className="text-xs text-[#CBD5E1]">Global</span>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        {user.department
                          ? <span className="text-xs text-[#334155]">{user.department}</span>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-[#94A3B8] whitespace-nowrap">{relativeTime(user.lastLogin)}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => handleEdit(user)} className="p-1.5 rounded-lg text-[#94A3B8] hover:text-[#6366F1] hover:bg-[#EEF2FF] transition-colors opacity-0 group-hover:opacity-100" title="Edit">
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button
                            onClick={() => handleToggleLock(user)}
                            className={clsx('p-1.5 rounded-lg transition-colors opacity-0 group-hover:opacity-100',
                              user.status === 'LOCKED' ? 'text-[#94A3B8] hover:text-[#10B981] hover:bg-[#ECFDF5]' : 'text-[#94A3B8] hover:text-[#F59E0B] hover:bg-[#FEF3C7]'
                            )}
        ...
        {!isLoading && users.length > 0 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-[#E2E8F0] bg-[#FAFBFC]">
            <span className="text-xs text-[#94A3B8]">
              Showing <span className="font-medium text-[#0F172A]">{(page - 1) * pageSize + 1}&ndash;{Math.min(page * pageSize, totalCount)}</span> of <span className="font-medium text-[#0F172A]">{totalCount}</span>
            </span>
            <div className="flex items-center gap-1">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className={clsx('p-1.5 rounded-lg transition-colors', page === 1 ? 'text-[#CBD5E1] cursor-not-allowed' : 'text-[#64748B] hover:text-[#0F172A] hover:bg-[#F1F5F9]')}>
                ...
                return (
                  <button key={p} onClick={() => setPage(p)}
                    className={clsx('w-8 h-8 rounded-lg text-xs font-medium transition-all',
                      page === p ? 'bg-[#0F172A] text-white' : 'text-[#64748B] hover:bg-[#F1F5F9]'
                    )}>
                    {p}
                  </button>
                );
              })}
              {totalPages > 5 && <span className="text-xs text-[#94A3B8] px-1">...</span>}
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
                className={clsx('p-1.5 rounded-lg transition-colors', page >= totalPages ? 'text-[#CBD5E1] cursor-not-allowed' : 'text-[#64748B] hover:text-[#0F172A] hover:bg-[#F1F5F9]')}>
