...
const roleDarkStyles: Record<string, React.CSSProperties> = {
  LEAD:     { background: 'rgba(217,119,6,0.15)',  color: '#FCD34D', border: '1px solid rgba(217,119,6,0.3)' },
  MEMBER:   { background: 'rgba(99,102,241,0.15)', color: '#A5B4FC', border: '1px solid rgba(99,102,241,0.3)' },
      ...
      <div className="relative rounded-2xl shadow-2xl w-full max-w-lg p-0 animate-slide-in" style={{ background: '#0C0A18', border: '1px solid rgba(139,92,246,0.25)' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold" style={{ color: '#E2EEF9' }}>Create New Team</h2>
              <p className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>Add a team to your organization</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          {/* Team type pills */}
          <div>
            <label className="block text-xs font-semibold mb-2 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Team Type</label>
              ...
              {Object.entries(TEAM_TYPES).filter(([k]) => k !== 'OTHER').map(([key, def]) => {
                ...
                return (
                  <button
                    ...
                    style={isActive ? { background: def.bg, color: def.hex, border: `1px solid ${def.border}` } : { background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.45)', border: '1px solid rgba(255,255,255,0.1)' }}
                  >
          ...
          <div>
            <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Team Name *</label>
            <input
          ...
          <div>
            <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Description</label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Team Email</label>
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: 'rgba(255,255,255,0.3)' }} />
                <input className="w-full pl-9 rounded-lg px-3 py-2 text-sm focus:outline-none" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#E2EEF9' }} placeholder="team@company.com" value={email} onChange={e => setEmail(e.target.value)} />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Slack Channel</label>
              <div className="relative">
                <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: 'rgba(255,255,255,0.3)' }} />
                <input className="w-full pl-9 rounded-lg px-3 py-2 text-sm focus:outline-none" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#E2EEF9' }} placeholder="team-channel" value={slackChannel} onChange={e => setSlackChannel(e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl text-sm font-medium transition-all" style={{ color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.1)' }}>Cancel</button>
          </div>
        </form>
      </div>
    </div>
...
function TeamCard({ team, expanded, onToggle }: { team: any; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="rounded-xl transition-all duration-200" style={{ background: 'rgba(255,255,255,0.04)', border: expanded ? `1px solid ${def.border}` : '1px solid rgba(255,255,255,0.08)' }}
      ...
      <div className="flex items-center justify-between p-5 cursor-pointer" onClick={onToggle}>
        <div className="flex items-center gap-4 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h3 className="text-[15px] font-semibold truncate" style={{ color: '#E2EEF9' }}>{team.name}</h3>
              ...
              {!team.isActive && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ background: 'rgba(220,38,38,0.15)', color: '#FCA5A5', border: '1px solid rgba(220,38,38,0.3)' }}>INACTIVE</span>}
            </div>
            <p className="text-xs mt-0.5 truncate" style={{ color: 'rgba(255,255,255,0.4)' }}>{team.description || 'No description'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 shrink-0 ml-4">
          ...
          <div className="text-center">
            <p className="text-lg font-bold font-display" style={{ color: '#E2EEF9' }}>{memberCount}</p>
            <p className="text-[10px] font-medium" style={{ color: 'rgba(255,255,255,0.4)' }}>Members</p>
          </div>

          {/* Workload indicator */}
          <div className="text-center">
            <p className="text-lg font-bold font-display" style={{ color: workloadColor }}>{totalWorkload}</p>
            <p className="text-[10px] font-medium" style={{ color: 'rgba(255,255,255,0.4)' }}>Workload</p>
          </div>

          {/* Contact icons */}
          <div className="flex items-center gap-1.5">
            {team.email && (
              <div className="p-1.5 rounded-md" style={{ background: 'rgba(255,255,255,0.07)' }} title={team.email}>
                <Mail className="w-3 h-3" style={{ color: 'rgba(255,255,255,0.4)' }} />
              </div>
            )}
            {team.slackChannel && (
              <div className="p-1.5 rounded-md" style={{ background: 'rgba(255,255,255,0.07)' }} title={`#${team.slackChannel}`}>
                <MessageSquare className="w-3 h-3" style={{ color: 'rgba(255,255,255,0.4)' }} />
              </div>
            )}
          </div>
      ...
      {/* Expanded content */}
      {expanded && (
        <div className="animate-fade-in" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          {/* Stats bar */}
          <div className="flex items-center gap-6 px-5 py-3" style={{ background: 'rgba(255,255,255,0.02)' }}>
            {managerName && (
              <span className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
                <Crown className="w-3 h-3" style={{ color: '#FCD34D' }} /> Manager: <span className="font-medium" style={{ color: '#CBD5E1' }}>{managerName}</span>
              </span>
            )}
            <span className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
              <AlertTriangle className="w-3 h-3" style={{ color: '#FCA5A5' }} /> <span className="font-medium" style={{ color: '#CBD5E1' }}>{incidentCount}</span> Incidents
            </span>
            <span className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
              <Zap className="w-3 h-3" style={{ color: '#A5B4FC' }} /> <span className="font-medium" style={{ color: '#CBD5E1' }}>{changeCount}</span> Changes
            </span>
            <span className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
              <Activity className="w-3 h-3" style={{ color: '#C4B5FD' }} /> <span className="font-medium" style={{ color: '#CBD5E1' }}>{problemCount}</span> Problems
            </span>
            <span className="flex items-center gap-1.5 text-xs ml-auto" style={{ color: 'rgba(255,255,255,0.35)' }}>
          ...
          <div className="p-5">
            {members.length === 0 ? (
              <div className="text-center py-8" style={{ color: 'rgba(255,255,255,0.4)' }}>
                ...
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.3)' }}>Add team members to get started</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                {members.map((member: any) => (
                  <div key={member.id} className="flex items-center gap-3 p-3 rounded-xl transition-all" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate" style={{ color: '#CBD5E1' }}>{member.name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={roleDarkStyles[member.role] || roleDarkStyles.MEMBER}>
                          {member.role}
                        </span>
                        {member.jobTitle && <span className="text-[10px] truncate" style={{ color: 'rgba(255,255,255,0.35)' }}>{member.jobTitle}</span>}
                      </div>
                    </div>
...
export default function TeamList() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64" style={{ background: 'linear-gradient(180deg, #0C0618 0%, #080410 40%, #060606 100%)', minHeight: '100vh' }}>
        <Loader2 className="w-8 h-8 animate-spin" style={{ color: '#8B5CF6' }} />
        <span className="ml-3 text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>Loading teams...</span>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center h-64" style={{ background: 'linear-gradient(180deg, #0C0618 0%, #080410 40%, #060606 100%)', minHeight: '100vh', color: '#FCA5A5' }}>
        <AlertTriangle className="w-10 h-10 mb-3" />
        <p className="text-lg font-semibold">Failed to load teams</p>
        <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Please check your connection and try again.</p>
      </div>
    );
  }

  return (
    <div className="animate-fade-in space-y-0" style={{ background: 'linear-gradient(180deg, #0C0618 0%, #080410 40%, #060606 100%)', minHeight: '100vh', padding: '1.5rem' }}>
      {/* ── HERO BANNER ── */}
      <div className="relative rounded-2xl overflow-hidden mb-5" style={{ background: 'linear-gradient(180deg, #0C0618 0%, #140824 100%)', border: '1px solid rgba(139,92,246,0.15)' }}>
        ...
        <div className="relative px-6 py-5">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2.5 mb-1.5">
                ...
                <h1 className="font-display text-2xl font-bold tracking-tight" style={{ color: '#E2EEF9' }}>Teams</h1>
              ...
              <p className="text-sm ml-[42px]" style={{ color: 'rgba(255,255,255,0.45)' }}>Manage teams, members, and on-call assignments across your organization</p>
            </div>
            {canManage && (
              <button
          ...
          <div className="flex items-center gap-4 mt-4 ml-[42px]">
            {[
              ...
              <div key={s.label} className="flex items-center gap-3 px-4 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div>
                ...
                <span className="text-[9px] font-mono" style={{ color: 'rgba(255,255,255,0.35)' }}>{s.sub}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Filter bar ── */}
      <div className="flex items-center gap-3 mb-4 flex-wrap p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', backdropFilter: 'blur(8px)' }}>
        {/* Search */}
        ...
        <span className="text-xs ml-auto font-mono" style={{ color: 'rgba(255,255,255,0.35)' }}>{filtered.length} team{filtered.length !== 1 ? 's' : ''}</span>
      </div>

      {/* ── Team cards ── */}
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          <Users className="w-12 h-12 mx-auto mb-3" style={{ color: 'rgba(255,255,255,0.2)' }} />
          <p className="text-lg font-semibold" style={{ color: 'rgba(255,255,255,0.5)' }}>No teams found</p>
