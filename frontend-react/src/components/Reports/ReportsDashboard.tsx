...
const TT = {
  contentStyle: {
    ...
    borderRadius: '10px', fontSize: '11px', color: '#CBD5E1',
...
function Skeleton({ className = '' }: { className?: string }) {
  return <div className={clsx('animate-pulse rounded-lg', className)} style={{ background: 'rgba(255,255,255,0.06)' }} />;
}

// Color token map for KpiCard accent colors
const KPI_ACCENT: Record<string, { color: string; bg: string }> = {
  'bg-indigo-500':  { color: '#818CF8', bg: 'rgba(99,102,241,0.15)' },
  'bg-amber-500':   { color: '#FCD34D', bg: 'rgba(217,119,6,0.15)' },
  'bg-red-500':     { color: '#FCA5A5', bg: 'rgba(220,38,38,0.15)' },
  'bg-red-400':     { color: '#FCA5A5', bg: 'rgba(220,38,38,0.12)' },
  'bg-violet-500':  { color: '#C4B5FD', bg: 'rgba(139,92,246,0.15)' },
  'bg-violet-400':  { color: '#C4B5FD', bg: 'rgba(139,92,246,0.12)' },
  'bg-emerald-500': { color: '#6EE7B7', bg: 'rgba(5,150,105,0.15)' },
  'bg-sky-500':     { color: '#7DD3FC', bg: 'rgba(14,165,233,0.15)' },
  'bg-rose-500':    { color: '#FCA5A5', bg: 'rgba(244,63,94,0.15)' },
};
...
function KpiCard({
  ...
  const accent = KPI_ACCENT[color] || { color: '#A5B4FC', bg: 'rgba(99,102,241,0.12)' };
  ...
  return (
    <div className="rounded-2xl p-5 relative overflow-hidden transition-all duration-200 hover:-translate-y-0.5"
      ...
      {loading ? (
        ...
        <>
          <div className="text-[28px] font-extrabold leading-none font-mono tracking-tight mb-1.5" style={{ color: '#E2EEF9' }}>
            {value}
          </div>
          <div className="text-[11px] font-medium" style={{ color: 'rgba(255,255,255,0.4)' }}>{label}</div>
...
function SectionHeader({ title, subtitle, loading }: { title: string; subtitle?: string; loading?: boolean }) {
  return (
    <div className="flex items-center justify-between mb-5">
      <div>
        <h2 className="text-base font-bold" style={{ color: '#E2EEF9' }}>{title}</h2>
...
function ChartCard({ title, subtitle, children, loading, className = '' }: {
  ...
  return (
    <div className={clsx('rounded-2xl p-5', className)} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-[13px] font-bold" style={{ color: '#E2EEF9' }}>{title}</h3>
          {subtitle && <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>{subtitle}</p>}
        </div>
        {loading && <Loader2 size={12} className="animate-spin" style={{ color: '#818CF8' }} />}
      </div>
      {children}
    </div>
  );
...
function EmptyChart({ message = 'No data available for this period' }: { message?: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-2" style={{ color: 'rgba(255,255,255,0.25)' }}>
      <BarChart3 size={28} />
...
function SlaRing({ value, label, size = 90 }: { value: number; label: string; size?: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>{label}</span>
    </div>
  );
}
...
function SLASection({ period }: { period: Period }) {
  ...
  return (
    <div className="space-y-6">
      {/* Overall SLA score */}
      <div className="bg-gradient-to-r from-[#0F172A] to-[#1E293B] rounded-2xl p-6 flex items-center gap-8">
        <div>
          <p className="text-slate-400 text-xs font-semibold uppercase tracking-widest mb-1">Overall SLA Score</p>
          <p className="text-[52px] font-extrabold font-mono leading-none text-white">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {teams.map((t: any) => {
                  ...
                  return (
                    <tr key={name} className="border-b border-stone-50 hover:bg-indigo-50/30 transition-colors group">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#0F172A] to-[#1E293B] rounded-2xl p-6 flex items-center gap-8">
        <div>
          ...
          <p className="text-[52px] font-extrabold font-mono leading-none text-white">{successPct}%</p>
        ...
        <div className="flex gap-8 text-slate-400 text-sm">
          {[
            ...
            <div key={l as string}>
              <p className="text-2xl font-bold text-white font-mono">{fmt(v as number)}</p>
      ...
      <div className="relative rounded-2xl overflow-hidden bg-[#0F172A] mb-5">
        ...
        <div className="relative px-6 py-5">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 bg-white/[0.06] rounded-lg border border-white/[0.08] p-0.5">
                {(['7d', '30d', '90d'] as Period[]).map(p => (
                  <button key={p} onClick={() => setPeriod(p)}
                    className={clsx('px-3 py-1 rounded-md text-xs font-semibold transition-all',
                      period === p ? 'bg-indigo-500/30 text-indigo-300' : 'text-slate-400 hover:text-white')}>
                    {p}
                  </button>
                ))}
              </div>
              <button onClick={handleExport}
                className="flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-indigo-500 to-violet-500 text-white rounded-xl text-xs font-bold shadow-lg hover:shadow-indigo-500/25 transition-all hover:scale-[1.02]">
          ...
          <nav className="bg-white rounded-2xl border border-stone-200 overflow-hidden sticky top-4">
            ...
            {SECTIONS.map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => setSection(id)}
                ...
                <Icon size={13} className={section === id ? 'text-indigo-500' : 'text-stone-400 group-hover:text-stone-600'} />
                {label}
                {section === id && <ChevronRight size={10} className="ml-auto text-indigo-400" />}
              </button>
            ))}
