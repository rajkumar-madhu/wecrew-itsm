...
function ProgressBar({ value, className = '' }: { value: number; className?: string }) {
  const color = value > 85 ? 'bg-red-500' : value > 70 ? 'bg-amber-500' : 'bg-indigo-500';
  return (
  ...
  return (
    <div ref={containerRef} className="relative bg-white rounded-xl border border-stone-200 shadow-sm group" style={{ minHeight: `calc(${iframeHeight} + 41px)` }}>
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-stone-100 bg-stone-50/50">
        ...
        <a href={src} target="_blank" rel="noopener noreferrer" className="opacity-0 group-hover:opacity-100 transition-opacity text-stone-400 hover:text-indigo-600">
...
export default function MetricsDashboard() {
  ...
  return (
    <div className="space-y-4">
      {/* ── HERO BANNER ── */}
      <div className="relative rounded-2xl overflow-hidden bg-[#0F172A] mb-1">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex items-center gap-3 ml-[42px] mt-0.5 flex-wrap">
                <p className="text-[#94A3B8] text-sm">Real-time Prometheus metrics across CPU, virtualization, storage & Grafana panels</p>
                {orgServerIp && (
                  <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-[11px] font-mono">
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 bg-white/[0.06] rounded-lg p-0.5 border border-white/[0.08]">
                <Clock className="w-3.5 h-3.5 text-[#94A3B8] ml-2" />
                {TIME_RANGES.map((tr) => (
                  <button key={tr.value} onClick={() => { setTimeRange(tr.value); setRefreshKey((k) => k + 1); }}
                    className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${timeRange === tr.value ? 'bg-indigo-500/20 text-indigo-300' : 'text-[#94A3B8] hover:text-white'}`}
                  >{tr.label}</button>
                ))}
              </div>
              <button onClick={handleRefresh} className="p-1.5 rounded-lg border border-white/[0.08] text-[#94A3B8] hover:text-indigo-400 hover:border-indigo-500/30 transition-all bg-white/[0.04]" title="Refresh">
      ...
      <div className="h-0.5 bg-gradient-to-r from-transparent via-indigo-500/60 to-transparent -mt-1 mb-3" />

      {/* Tabs */}
      <div className="flex items-center gap-1 border-b border-stone-200 overflow-x-auto">
        {TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-all -mb-px whitespace-nowrap ${activeTab === tab ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300'}`}
          >{tab}</button>
        ))}
      </div>
      ...
      {activeTab === 'Overview' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={Cpu} label="CPU Usage" value={`${cpu.avgUsagePct}%`} sub={`${cpu.totalCores} cores total`} color="bg-indigo-500" />
      ...
      {activeTab === 'Virtualization' && (
        <div className="space-y-5">
          {virt.nodes.length === 0 && virt.totalPods === 0 && virt.deployments.length === 0 && (
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-indigo-200 bg-indigo-50 text-indigo-800 text-sm">
            ...
            <Section title="Pod Status Distribution" icon={Container}>
              ...
              <div className="mt-4 p-3 bg-indigo-50 rounded-lg border border-indigo-100">
                <div className="flex justify-between">
                  <span className="text-xs font-medium text-indigo-600">Total Pods</span>
                  <span className="text-sm font-bold text-indigo-700">{virt.totalPods}</span>
                </div>
              </div>
            </Section>
              ...
              <div className="flex items-center gap-1 border-b border-stone-200">
                {dashboards.map((db, idx) => (
                  <button key={db.uid} onClick={() => setGrafanaTab(idx)}
                    className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-all -mb-px ${grafanaTab === idx ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
