...
const stateClass: Record<ProblemState, string> = {
  NEW: 'bg-[color:var(--argus-signal-dim)] text-[#4F46E5] border-[color:var(--argus-signal)]/25',
      ...
      <div className="relative rounded-2xl overflow-hidden bg-obsidian text-ink border border-[color:var(--argus-border)] mb-5">
        ...
        <div className="relative px-6 py-5">
          <div className="flex items-center gap-2 mb-4">
            <button onClick={() => navigate('/problems')} className="flex items-center gap-1.5 text-muted hover:text-ink text-sm transition-colors">
              <ArrowLeft size={14} /> Problems
            </button>
            <span className="text-[#475569]">/</span>
            <span className="text-muted font-mono text-sm">{prb.number}</span>
          </div>
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2.5 flex-wrap mb-3">
                ...
                {prb.category && <span className="text-[10px] bg-[color:var(--argus-elevated)] text-muted px-2 py-0.5 rounded border border-[color:var(--argus-border)]">{prb.category}</span>}
              </div>
              <h1 className="text-xl font-display font-bold text-ink">{prb.shortDescription}</h1>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={() => setShowEditModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-[color:var(--argus-elevated)] hover:bg-[color:var(--argus-elevated)] text-muted text-xs font-medium rounded-lg transition-all border border-[color:var(--argus-border)]">
                <Pencil size={13} /> Edit
              </button>
              <button onClick={() => setShowAssignModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-[color:var(--argus-elevated)] hover:bg-[color:var(--argus-elevated)] text-muted text-xs font-medium rounded-lg transition-all border border-[color:var(--argus-border)]">
        ...
        <div className="space-y-5">
          ...
          <div className="glass-card p-5 border-violet-200/50 bg-gradient-to-br from-violet-50/30 to-white">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 btn-primary disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {aiRCA.isPending ? (
                  <>
          ...
          <div className="glass-card p-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="text-center p-3 bg-stone-50 rounded-xl border border-stone-100">
                <Server size={18} className="mx-auto mb-1 text-[color:var(--argus-signal)]" />
          ...
          {(matchedKBEntries.length > 0 || alertKBEntries.length > 0) && (
            <div className="glass-card p-5">
              <h3 className="text-sm font-semibold text-stone-800 mb-3 flex items-center gap-2">
                <BookOpen size={14} className="text-[color:var(--argus-signal)]" />
                  ...
                  <div key={kb.key} className={clsx('rounded-xl border p-4', matchedKBEntries.includes(kb) ? 'border-emerald-200 bg-emerald-50/30' : 'border-stone-200 bg-stone-50/50')}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className="text-[10px] font-semibold text-[color:var(--argus-signal)] hover:text-indigo-800 hover:bg-[color:var(--argus-signal-dim)] px-2 py-0.5 rounded transition-all"
                      >
                        Apply to RCA
                      </button>
                    </div>
