...
const PRIORITY_META: Record<Priority, {
  ...
  P3: { label: 'Medium',    color: 'var(--argus-signal)', glow: 'rgba(96,165,250,0.28)',  ring: 'rgba(96,165,250,0.4)',  bg: 'rgba(96,165,250,0.08)',  gradFrom: '#60A5FA', gradTo: '#4F46E5', slaResponse: '1 hr',   slaResolution: '24 hr', slaFull: 'Resp 1h | Res 24h' },
  P4: { label: 'Low',       color: 'var(--argus-emerald)', glow: 'rgba(52,211,153,0.25)',  ring: 'rgba(52,211,153,0.4)',  bg: 'rgba(52,211,153,0.07)',  gradFrom: '#34D399', gradTo: '#059669', slaResponse: '4 hr',   slaResolution: '72 hr', slaFull: 'Resp 4h | Res 72h' },
};
...
const IMPACTS: { value: Impact; label: string; desc: string; color: string; glow: string }[] = [
  ...
  { value: 'TEAM',       label: 'Team',       desc: 'Team scope',         color: 'var(--argus-signal)', glow: 'rgba(96,165,250,0.22)' },
  { value: 'INDIVIDUAL', label: 'Individual', desc: 'Single user',        color: 'var(--argus-emerald)', glow: 'rgba(52,211,153,0.20)' },
];
...
const URGENCIES: { value: Urgency; label: string; desc: string; color: string; glow: string }[] = [
  ...
  { value: 'MEDIUM',   label: 'Medium',   desc: 'Standard response time',    color: 'var(--argus-signal)', glow: 'rgba(96,165,250,0.22)' },
  { value: 'LOW',      label: 'Low',      desc: 'Can be scheduled',          color: 'var(--argus-emerald)', glow: 'rgba(52,211,153,0.20)' },
];

// ─── Categories ───────────────────────────────────────────────────────────────
const CATEGORIES: { value: string; icon: React.ElementType; color: string }[] = [
  { value: 'Hardware',          icon: Monitor,   color: 'var(--argus-signal)' },
  { value: 'Software',          icon: FileText,  color: '#A78BFA' },
  { value: 'Network',           icon: Wifi,      color: 'var(--argus-emerald)' },
            ...
            <div className="flex flex-col items-center gap-1.5">
              <div className="relative">
                <div className={clsx('w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 border')}
                  ...
                  {done
                    ? <CheckCircle2 size={16} style={{ color: 'var(--argus-ink)' }} />
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: SURFACE, border: `1px solid ${BORDER_LT}` }}>
      ...
      <div className="flex items-center gap-3 px-6 py-4"
        ...
        <div className="w-9 h-9 rounded-xl flex items-center justify-center"
          ...
          <Icon size={16} style={{ color: 'var(--argus-ink)' }} />
        </div>
        <div>
    ...
    <>
      ...
      <div className="-m-6" style={{ background: BG, minHeight: 'calc(100vh - 56px)' }}>
        ...
        <div className="relative overflow-hidden px-8 py-6"
          style={{ background: 'var(--argus-surface)', borderBottom: `1px solid ${BORDER_LT}` }}>
          {/* Ambient blobs */}
          ...
          <nav className="relative flex items-center gap-1.5 text-[11px] mb-5">
            <button type="button" onClick={() => navigate('/dashboard')} className="transition-colors hover:text-ink" style={{ color: TEXT2 }}>WeCrew</button>
            <ChevronRight size={11} style={{ color: TEXT2 }} />
            <button type="button" onClick={() => navigate('/incidents')} className="transition-colors hover:text-ink" style={{ color: TEXT2 }}>Incidents</button>
            <ChevronRight size={11} style={{ color: TEXT2 }} />
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button type="button" onClick={() => navigate('/incidents')}
                ...
                style={{ background: 'var(--argus-elevated)', border: `1px solid ${BORDER_LT}` }}>
                  ...
                  <div className="rounded-2xl p-5 text-center transition-all duration-500"
                    ...
                    <div className="flex items-center justify-center mb-3">
                      <div className="w-16 h-16 rounded-full flex items-center justify-center"
                        ...
                        <span className="text-xl font-black text-ink">{priority}</span>
                      </div>
                    </div>
