...
function ProviderBadge({ provider }: { provider: SMSProvider }) {
  const cls: Record<SMSProvider, string> = {
    TWILIO: 'bg-[color:var(--argus-signal-dim)] text-signal border-[color:var(--argus-signal)]/25',
...
function StatusBadge({ status }: { status: string }) {
  ...
  const cls =
    ...
    s === 'RECEIVED' ? 'bg-[color:var(--argus-signal-dim)] text-signal border-[color:var(--argus-signal)]/25' :
...
export default function SMSDashboard() {
  ...
  return (
    <div className="animate-fade-in space-y-0">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2.5 mb-1.5">
                ...
                <h1 className="font-display text-2xl font-bold text-ink tracking-tight">SMS Gateway</h1>
              </div>
            ...
            <button onClick={() => setShowSendForm(!showSendForm)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[color:var(--argus-signal)] to-[color:var(--argus-signal-bright)] text-white rounded-xl text-sm font-semibold shadow-lg hover:shadow-indigo-500/25 transition-all duration-200 hover:scale-[1.02]">
      ...
      {/* Send SMS Form */}
      {showSendForm && (
        <div className="glass-card p-5 border-[color:var(--argus-signal)]/25 space-y-4 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-stone-50/80 border border-stone-200/80 rounded-lg text-sm text-stone-900 placeholder-stone-400 focus:outline-none focus:border-[color:var(--argus-signal)] focus:ring-2 focus:ring-indigo-100 transition-all"
            />
          </div>
        </div>
      </div>
