    ...
    <div className="relative flex-1" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      ...
      {hovered && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-10 whitespace-nowrap
          bg-obsidian text-ink border border-[color:var(--argus-border)] border border-[#334155] text-xs text-[#E2E8F0] px-2.5 py-1.5 rounded-lg shadow-xl pointer-events-none">
...
export default function StatusPage() {
  ...
  // ── Loading ──────────────────────────────────────────────────────────────────
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--argus-void)' }}>
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--argus-void)', fontFamily: 'Inter, system-ui, sans-serif' }}>

      {/* ── Top nav ── */}
      <header style={{ borderBottom: '1px solid #1E293B', background: 'var(--argus-surface)' }}>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-8">
          ...
          <div>
            ...
            <div className="space-y-3">
              {data.activeIncidents.map((inc) => (
                <div key={inc.id} className="rounded-xl p-4" style={{ background: 'var(--argus-elevated)', border: '1px solid #334155', borderLeft: `3px solid ${PRIORITY_COLORS[inc.priority] || '#6366F1'}` }}>
        ...
        {data.components.length === 0 && (
          <div className="rounded-xl p-8 text-center" style={{ background: 'var(--argus-elevated)', border: '1px solid #334155' }}>
        ...
        <div>
          ...
          <div className="rounded-xl p-4" style={{ background: 'var(--argus-elevated)', border: '1px solid #334155' }}>
