...
function CodeBlock({ code, language = 'bash' }: { code: string; language?: string }) {
  ...
  return (
    <div className="relative group rounded-xl overflow-hidden border border-[#1E293B] my-4">
      <div className="flex items-center justify-between px-4 py-2 bg-obsidian text-ink border border-[color:var(--argus-border)] border-b border-[#1E293B]">
      ...
      <pre className="p-4 bg-obsidian text-ink border border-[color:var(--argus-border)] overflow-x-auto text-[12px] leading-relaxed">
    ...
    <div className="my-4 flex flex-col gap-2">
      {[
        ...
        <div key={s.step} className="flex items-start gap-3 px-4 py-3 rounded-xl border border-[#E7E5E4] bg-[#FAFAF9]">
          <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[color:var(--argus-signal)]/10 text-[#4F46E5] text-[11px] font-mono font-bold flex items-center justify-center">{s.step}</span>
...
export default function DeveloperDocs() {
  ...
  return (
    <div className="flex h-screen bg-[#FAFAF9] font-body overflow-hidden">
      {/* ── Mobile Header ────────────────────────────── */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 bg-obsidian text-ink border border-[color:var(--argus-border)] border-b border-[#1E293B]">
      ...
      {/* ── Sidebar ─────────────────────────────────── */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-[280px] bg-obsidian text-ink border border-[color:var(--argus-border)] border-r border-[#1E293B] flex flex-col transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        {/* Logo */}
        ...
        <div className="px-4 py-3 border-b border-[#1E293B]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate text-ink/50 border border-[#1E293B] rounded-lg text-[12px] font-mono text-[#E2E8F0] placeholder-[#475569] focus:outline-none focus:border-[#4F46E5]/50 focus:ring-1 focus:ring-[#4F46E5]/20 transition-all"
            />
          </div>
        </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12.5px] font-medium transition-all ${
                        activeSection === section.id
                          ? 'bg-[color:var(--argus-signal)]/10 text-signal border border-[#4F46E5]/20'
                          : 'text-[#94A3B8] hover:text-white hover:bg-slate text-ink/50 border border-transparent'
                      }`}
                    >
                      <span className={activeSection === section.id ? 'text-signal' : 'text-[#475569]'}>{section.icon}</span>
                      {section.title}
                      {section.endpoints && (
      ...
      <main ref={contentRef} className="flex-1 overflow-y-auto pt-0 lg:pt-0">
        <div className="pt-[56px] lg:pt-0">
          {/* Hero Header */}
          <div className="bg-obsidian text-ink border border-[color:var(--argus-border)] px-6 sm:px-10 py-10 sm:py-14 border-b border-[#1E293B] relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[color:var(--argus-signal)]/[0.04] rounded-full blur-[120px] -translate-y-1/2 translate-x-1/4" />
            <div className="relative max-w-4xl">
              <div className="flex items-center gap-3 mb-4">
                <div className="px-2.5 py-1 rounded-md bg-[color:var(--argus-signal)]/10 border border-[#4F46E5]/20 text-[10px] font-mono font-bold text-signal uppercase tracking-wider">API Reference</div>
              ...
              <div className="flex flex-wrap gap-3 mt-6">
                <span className="px-3 py-1.5 rounded-lg bg-slate text-ink/50 border border-[#1E293B] text-[11px] font-mono text-[#94A3B8]">REST API</span>
                <span className="px-3 py-1.5 rounded-lg bg-slate text-ink/50 border border-[#1E293B] text-[11px] font-mono text-[#94A3B8]">JSON</span>
                <span className="px-3 py-1.5 rounded-lg bg-slate text-ink/50 border border-[#1E293B] text-[11px] font-mono text-[#94A3B8]">JWT Auth</span>
                <span className="px-3 py-1.5 rounded-lg bg-slate text-ink/50 border border-[#1E293B] text-[11px] font-mono text-[#94A3B8]">Multi-tenant</span>
                <span className="px-3 py-1.5 rounded-lg bg-slate text-ink/50 border border-[#1E293B] text-[11px] font-mono text-[#94A3B8]">WebSocket</span>
              </div>
            </div>
          </div>
            ...
            {allSections.map(section => (
              <section key={section.id} id={`doc-${section.id}`} className="mb-16 scroll-mt-20 lg:scroll-mt-8">
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#E7E5E4]">
                  <div className="w-8 h-8 rounded-lg bg-[color:var(--argus-signal)]/8 flex items-center justify-center text-[#4F46E5]">{section.icon}</div>
                  <h2 className="font-display font-bold text-[22px] text-[color:var(--argus-ink)]">{section.title}</h2>
                  {section.endpoints && (
