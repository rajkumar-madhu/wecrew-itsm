...
const typeClass: Record<AssetType, string> = {
  SERVER: 'bg-[color:var(--argus-signal-dim)] text-[color:var(--argus-signal)] border-[color:var(--argus-signal)]/25',
  ...
  NETWORK: 'bg-[color:var(--argus-signal-dim)] text-[color:var(--argus-signal)] border-[color:var(--argus-signal)]/25',
  STORAGE: 'bg-amber-50 text-amber-600 border-amber-200',
  CONTAINER: 'bg-violet-50 text-violet-600 border-violet-200',
  VM: 'bg-[color:var(--argus-signal-dim)] text-[color:var(--argus-signal)] border-[color:var(--argus-signal)]/25',
  ...
  return (
    <div className="glass-card p-5">
      <h3 className="text-sm font-semibold text-stone-700 mb-4 flex items-center gap-2">
        <MonitorSpeaker size={15} className="text-[color:var(--argus-signal)]" /> System Information
      </h3>
            ...
            {interfaces.map((iface: any) => (
              <tr key={iface.device} className="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                <td className="py-2.5 px-2">
                  <div className="flex items-center gap-2">
                    <Network size={12} className="text-signal" />
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      <div className="glass-card p-4">
        <div className="flex items-center gap-2 mb-1.5">
          <Network size={14} className="text-[color:var(--argus-signal)]" />
    ...
    <div className="bg-stone-50 rounded-lg p-3">
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          {iface.isWan && <span className="text-[9px] bg-coral-dim text-[color:var(--argus-coral)] px-1.5 py-0.5 rounded-full font-medium">WAN</span>}
...
function DiskIOCard({ diskIO }: { diskIO: any[] }) {
  if (!diskIO || diskIO.length === 0) {
    return (
      <div className="glass-card p-5">
        <h3 className="text-sm font-semibold text-stone-700 mb-3 flex items-center gap-2">
          <Activity size={15} className="text-[color:var(--argus-signal)]" /> Disk I/O
        </h3>
  ...
  return (
    <div className="glass-card p-5">
      <h3 className="text-sm font-semibold text-stone-700 mb-4 flex items-center gap-2">
        <Activity size={15} className="text-[color:var(--argus-signal)]" /> Disk I/O Performance
  ...
  if (isLoading) {
    return (
      <div className="glass-card p-12 flex flex-col items-center gap-3">
        <Loader2 size={28} className="text-[color:var(--argus-signal)] animate-spin" />
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-stone-700 flex items-center gap-2">
          <Clock size={15} className="text-[color:var(--argus-signal)]" /> Metric Trends
        </h3>
        <div className="flex gap-1 bg-stone-100 rounded-lg p-0.5">
          {durations.map(d => (
            <button key={d.value} onClick={() => setDuration(d.value)}
              className={clsx('px-3 py-1 text-xs font-medium rounded-md transition-all',
                duration === d.value ? 'bg-white text-[color:var(--argus-signal)] shadow-sm' : 'text-stone-500 hover:text-stone-700')}>
              {d.label}
            </button>
          ))}
        </div>
...
function AIAnalysisCard({ analysis, issues, recommendations }: { analysis: string; issues: any[]; recommendations: any[] }) {
  return (
    <div className="glass-card p-5 border-l-4 border-l-indigo-400">
      <h3 className="text-sm font-semibold text-stone-700 mb-3 flex items-center gap-2">
        <Zap size={15} className="text-[color:var(--argus-signal)]" /> AI Analysis
      </h3>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {recommendations.map((rec: any, i: number) => (
              <div key={i} className="flex items-start gap-2 text-xs bg-[color:var(--argus-signal-dim)] rounded-lg p-2">
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="glass-card p-16 flex flex-col items-center justify-center gap-3">
          <Loader2 size={32} className="text-[color:var(--argus-signal)] animate-spin" />
  ...
  return (
    <div className="animate-fade-in space-y-0">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-obsidian text-ink border border-[color:var(--argus-border)] mb-5">
        ...
        <div className="relative px-6 py-5">
          <div className="flex items-center gap-2 mb-4">
            <button onClick={() => navigate('/assets')} className="flex items-center gap-1.5 text-slate-400 hover:text-ink text-sm transition-colors">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="flex-1">
              ...
              <h1 className="text-xl font-display font-bold text-ink">{asset.name || '-'}</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowEditModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-[color:var(--argus-elevated)] border border-[color:var(--argus-border)] text-slate-300 hover:text-ink hover:bg-[color:var(--argus-elevated)] transition-all">
                <Pencil size={14} /> Edit
              </button>
            </div>
          </div>
      ...
      <div className="flex gap-1 border-b border-stone-200 overflow-x-auto">
        {tabs.map((tab) => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={clsx('px-4 py-2.5 text-sm font-medium transition-colors relative flex items-center gap-1.5 whitespace-nowrap', activeTab === tab.key ? 'text-[color:var(--argus-coral)]' : 'text-stone-400 hover:text-stone-700')}>
            <tab.icon size={14} />
            {tab.label}
            {'count' in tab && tab.count > 0 && <span className={clsx('text-[10px] px-1.5 py-0.5 rounded-full', activeTab === tab.key ? 'bg-coral-dim text-[color:var(--argus-coral)]' : 'bg-stone-100 text-stone-500')}>{tab.count}</span>}
            {activeTab === tab.key && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-coral rounded-full" />}
          </button>
        ))}
      </div>
              ...
              {relatedIncidents.map((inc: any) => (
                <button key={inc.id} onClick={() => navigate(`/incidents/${inc.id}`)} className="w-full text-left p-3 rounded-lg bg-stone-50 hover:bg-stone-100 transition-colors flex items-center justify-between group">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="font-mono text-[color:var(--argus-signal)] text-xs shrink-0">{inc.number}</span>
                ...
                <div className="space-y-6">
                  {asset.activities.map((event: any, i: number) => (
                    <div key={i} className="flex gap-4 relative">
                      <div className="w-8 h-8 rounded-full bg-stone-200 flex items-center justify-center z-10 text-[color:var(--argus-signal)]">
                        <Clock size={14} />
                      </div>
