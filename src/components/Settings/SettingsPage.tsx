  ...
  return (
    <div className="animate-fade-in space-y-0">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-obsidian text-ink border border-[color:var(--argus-border)] mb-5">
        ...
        <div className="absolute top-0 right-0 w-80 h-80 bg-[color:var(--argus-signal)]/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4" />
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2.5 mb-0.5">
                ...
                <span className="text-[9px] font-mono font-bold text-signal bg-[color:var(--argus-signal)]/15 px-1.5 py-0.5 rounded border border-[#6366F1]/20">{user?.role}</span>
              </div>
              <p className="text-muted text-sm">{user?.email} &middot; {user?.timezone || 'Asia/Kolkata'}</p>
            </div>
          </div>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className={clsx('w-8 h-8 rounded-lg flex items-center justify-center shrink-0',
                  activeTab === tab.id ? 'bg-[color:var(--argus-signal)] text-white' : 'bg-[#F1F5F9] text-muted'
                )}>
                  <Icon className="w-4 h-4" />
                </div>
                <div className="min-w-0">
                  <p className={clsx('text-sm font-medium truncate', activeTab === tab.id ? 'text-[color:var(--argus-ink)]' : 'text-muted')}>{tab.label}</p>
                  <p className="text-[10px] text-muted truncate">{tab.desc}</p>
                </div>
              </button>
            );
          })}
                  ...
                  {[
                    ...
                    <div key={item.label} className="flex items-center justify-between py-3 px-4 rounded-lg hover:bg-[#FAFBFC] transition-colors">
                      ...
                      <div className={clsx(
                        ...
                        item.default ? 'bg-[color:var(--argus-signal)]' : 'bg-[#CBD5E1]'
                      )}>
                        <div className={clsx(
              ...
              <Section title="Theme" description="Choose your preferred interface theme">
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    { id: 'dark', label: 'Dark', icon: Moon, desc: 'Easy on the eyes', selected: false, preview: 'bg-obsidian text-ink border border-[color:var(--argus-border)] border-[#334155]' },
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <Icon className={clsx('w-4 h-4', theme.selected ? 'text-[#6366F1]' : 'text-muted')} />
                          <div>
              ...
              <div className="space-y-2">
                {orgs.map((org: any) => (
                  ...
                  >
                    ...
                    <ChevronRight className="w-4 h-4 text-ink group-hover:text-[#6366F1] transition-colors" />
                  </div>
                ))}
                {orgs.length === 0 && (
      ...
      {orgModal !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setOrgModal(null)}>
          <div className="absolute inset-0 bg-obsidian text-ink border border-[color:var(--argus-border)]/50 backdrop-blur-sm" />
          <div className="relative bg-white rounded-2xl shadow-xl border border-[#E2E8F0] p-6 w-full max-w-lg animate-slide-in" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setOrgModal(null)} className="w-8 h-8 rounded-lg hover:bg-[#F1F5F9] flex items-center justify-center text-muted hover:text-[color:var(--argus-ink)] transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
