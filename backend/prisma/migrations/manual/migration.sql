...
CREATE INDEX IF NOT EXISTS idx_incidents_state_priority
  ON "Incident" ("state", "priority");

CREATE INDEX IF NOT EXISTS idx_incidents_assigned_state
  ON "Incident" ("assignedToId", "state") WHERE "assignedToId" IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_incidents_group_state
...
CREATE INDEX IF NOT EXISTS idx_incidents_created_desc
  ON "Incident" ("createdAt" DESC);

-- Changes: common query patterns
CREATE INDEX IF NOT EXISTS idx_changes_state_type
  ON "Change" ("state", "type");

CREATE INDEX IF NOT EXISTS idx_changes_planned_dates
  ON "Change" ("plannedStartDate", "plannedEndDate");

-- Problems: common query patterns
CREATE INDEX IF NOT EXISTS idx_problems_known_error
  ON "Problem" ("isKnownError") WHERE "isKnownError" = true;

CREATE INDEX IF NOT EXISTS idx_problems_state_priority
...
CREATE INDEX IF NOT EXISTS idx_emailqueue_pending
  ON "EmailQueue" ("status", "scheduledAt") WHERE "status" = 'PENDING';

-- SMS Log: recent logs
CREATE INDEX IF NOT EXISTS idx_smslog_created_desc
  ON "SMSLog" ("createdAt" DESC);

-- Voice Call Log: recent logs
CREATE INDEX IF NOT EXISTS idx_voicecalllog_created_desc
